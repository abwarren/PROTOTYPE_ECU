...
Cross-platform mobile app for diagnostics, monitoring, and basic tuning via BLE/WiFi. Branding is loaded from `branding/brand.json` at build time.
...
```yaml
# CI injects brand configuration
- run: flutter build --dart-define=BRAND_CONFIG=$(cat branding/brand.json)
```

The app reads:
- `mobile.app_display_name` for app name
- `mobile.app_icon` for app icon
- `mobile.splash_screen` for splash screen
- `mobile.primary_color` for theme color
